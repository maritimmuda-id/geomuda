        ...
        <div className="flex gap-x-10 justify-between w-full">
          <div className="relative z-[1]">
            <Image
              ...
              src={dataKta?.photo ? dataKta?.photo : !imageUpload ? assets.defaultImage : URL.createObjectURL(imageUpload)}
              width={500}
              height={500}
            />

            {!validate && (
              <button className="w-9 aspect-square bg-white border-2 rounded-full absolute -bottom-3 -right-4" onClick={setKta}>
          ...
          <div className="flex flex-col space-y-4 justify-between w-full">
            <div className="flex flex-col space-y-2 w-full">
              <label>Nama</label>
              <input type="text" value={user_metadata.fullname} disabled className="border-2 border-[#00000066] px-3 py-[6px] p-1 rounded-lg w-full disabled:cursor-not-allowed" />
            </div>
            <div className="flex flex-col space-y-2 w-full">
              <label>Nomor KTP</label>
              <div className="flex gap-x-6">
                <input type="text" disabled={validate} value={nik} onChange={(e) => setNik(e.target.value)} className="border-2 border-[#00000066] px-3 py-[6px] p-1 rounded-lg w-full disabled:cursor-not-allowed" />

                {!validate && (
                  <button onClick={handleCekNik} className="bg-[#7B2418] text-white p-1 rounded-lg w-1/5">
                    Cek
                  </button>
                )}
              </div>
            </div>
            <button disabled={!valid || !imageUpload || validate} onClick={handleSubmit} className="bg-[#7B2418] text-white p-2 rounded-lg w-full disabled:cursor-not-allowed disabled:bg-gray-400">
              {loadingGenerate ? 'Loading...' : validate ? 'KTA Telah Digenerate' : 'Buat KTA'}
            </button>
          </div>
        </div>
      </div>
