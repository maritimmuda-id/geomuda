    ...
    <div>
      ...
      <div className='-mt-24 flex flex-col justify-center items-center'>
        ...
        <div className='mt-8 mb-16 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-y-12 gap-x-12 mx-20'>
          {
            news.map((data, index) => (
              <div className='w-full mx-auto  bg-slate-50 p-6 rounded-md border-2' key={index}>
                <p className='uppercase text-sm text-gray-600'>{data.type}</p>
                <h2 className='capitalize text-xl font-semibold my-2'>{data.title}</h2>
                <p className='text-sm leading-5 line-clamp-[8] my-5'>{data.description}</p>
